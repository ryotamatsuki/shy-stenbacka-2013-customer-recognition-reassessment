...
def positive_root(expr):
    roots=[x for x in sp.solve(sp.Eq(expr,0),z) if x.is_real and float(sp.N(x))>0]
...
with (OUT/"key_thresholds.tex").open("w",encoding="utf-8") as f:
    ...
    f.write("Object & Normalized value \\\\\\\\\\midrule\\n")
...
with (OUT/"results_map.tex").open("w",encoding="utf-8") as f:
    ...
    f.write("\\begin{tabular}{cl}\\toprule\\nResult & Frozen disposition \\\\\\\\\\midrule\\n")
    for a,b in items:
        f.write(f"{a} & {b} \\\\\\\\\\n")
    f.write("\\bottomrule\\end{tabular}\\n")
